...
'''A new class called CapsuleSearchAgent will be created here
Just copy from the previous code
Change class name to CapsuleSearchAgent'''
...
'''
A new class CapsuleSearchProblem
Two functions will be modified
(1) getSuccessors
(2) foodHeuristic
'''
...
def foodHeuristic(state, problem):
    ...
    result = 0
    distanceToFood = []
    foodProblem = problem.startingGameState

    if visitStatus:
        prob = PositionSearchProblem(foodProblem, start=position, goal=problem.capsule, warn=False,
                                     visualize=False)
        distanceToFood.append(manhattanHeuristic(position, prob))
    else:
